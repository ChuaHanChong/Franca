...
def _generate_randperm_indices(*, size: int, generator: torch.Generator):
    ...
    # This is actually matching PyTorch's CPU implementation,
...
def _shuffle_tensor_slice(*, tensor: torch.Tensor, start: int = 0, step: int = 1, generator: torch.Generator) -> np.ndarray:
    stop = len(tensor)
    count = stop // step
...
class ShardedInfiniteSampler(Sampler):
    def __init__(
        ...
        *,
        ...
        self._advance = advance
        self._iter_count = 0
        self._shuffle_tensor_slice_fn = _new_shuffle_tensor_slice if use_new_shuffle_tensor_slice else _shuffle_tensor_slice
    ...
    def _shuffled_iterator(self):
        ...
        while True:
            ...
            iterable = self._shuffle_tensor_slice_fn(tensor=perm, start=self._start, step=self._step, generator=generator)
            yield from iterable
            self._iter_count += 1
